...
class CheckoutPaymentButton extends Component {
  ...
  payAction() {
    ...
    let startTime2 = Moment(data.startDate).format("YYYY-MM-DD HH:mm");
    let realEndTime = Moment(data.endDate).format("YYYY-MM-DD HH:mm");
    ...
    let roundedPrice = Number.parseFloat(data.price).toFixed(2);
  ...
  render() {
    ...
    return (
      ...
      <div style={{marginTop: 12, marginBottom: 40}}>
        ...
        <Button variant="contained" color='primary' fullWidth onClick={this.handlePayAction} disabled={this.props.disabled}>
          {" "}
          Confirm and Pay
        </Button>
      </div>
